...
function setForeCastData(data) {
    ...
    data.forEach((data, index) => {
        ...
        foreCast.innerHTML = ``;
            ...
            const iconId = data.day.condition.icon.substr("//cdn.weatherapi.com/weather/64x64/day/".length);
            let li = document.createElement("li");
            li.innerHTML = `<span>${data.date}</span>
                            <img src="${"//cdn.weatherapi.com/weather/64x64/day/" + iconId}"
                            alt="icon"
                            width="50"
                            height="50"
                            />
...
function fetchWeatherData(cityInput) {
    fetch(weatherApiUrl + `&q=${cityInput}`).then(res => res.json()).then(data => {
        ...
        .catch((e) => {
            ...
            if(e == "TypeError: Failed to fetch"){
                alert("Internet is not available, pls Connect your device on internet");
            }else{
